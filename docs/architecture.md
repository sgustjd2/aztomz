...
```
index.html      홈 — 마스트헤드 · 피처드 · "광고일까 진짜일까" 랭킹 인덱스 · "요즘 트렌드" 스트립
trend.html?id=  상세 — 점수(광고/신뢰)·만족도·한끗 판단·추천·출처·방문자 후기(작성/목록)·함께 보기
login.html      로그인 (?next= 리다이렉트)
signup.html     회원가입 (?next=)
me.html         내 한끗 — 저장한 트렌드 · 내가 쓴 후기 · 로그아웃 (비로그인 시 login으로 가드)
assets/styles.css  에디토리얼 디자인 시스템 (DESIGN.md 참고)
assets/app.js      목 스토어(H.*) + 신선도 계산 + 마스트헤드/리뷰/저장 헬퍼
data/trends.json   ★ 데이터 원본(canonical). 사람/스크립트가 수정.
data/trends.js     ← refresh.mjs가 trends.json에서 생성(편집 금지). 사이트가 로드.
scripts/refresh.mjs  일일 갱신: 재수집·재분석(hermes 플러그) + generatedAt=오늘 + trends.js 생성
...
- 흐름: `trends.json`(원본) → `refresh.mjs`(hermes 재수집·재분석 + 날짜 스탬프) → `trends.js`(생성) → 사이트.
  로컬 수동 실행: `node scripts/refresh.mjs`. 운영: GitHub Action 크론(매일 06:00 KST).
- hermes 연결 지점: `scripts/refresh.mjs`의 `collectAndAnalyze()` (현재 no-op TODO).

## 다음에 붙일 만한 리텐션 기능
- 저장 알림 / "이번 주 한끗" 구독(이메일·푸시)
- hermes가 매일 `trends` 자동 upsert → 콘텐츠 신선도(재방문 동력)
- 후기 좋아요·신고, 작성자 평판
- 개인화: 저장·후기 기반 추천
