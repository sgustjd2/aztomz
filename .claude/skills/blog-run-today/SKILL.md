...
> ⛔ **일일 물량 배치 중단 중 (2026-09-13 결정).** "오늘 작업 진행"이 와도 5편 배치를 그냥 돌리지 마라.
> 근거: 9월 1~13일 71편 발행 → 전부 조회수 1, 애드센스 $0.01/일, 네이버 검색 유입 0, 배치당 서브에이전트 토큰 ~114만(매일 적자).
> 재개 조건: (1) 네이버 서치어드바이저·구글 서치콘솔 색인 상태 진단 완료 — GSC는 2026-09-13 진단(미색인 613·중복판정 195), 네이버 서치어드바이저 등록/제출은 **사람이 로그인해 확인**(seo-playbook §3). (2) 주제 선정에 **검색수요 게이트**가 박힘 — ✅ 게이트 스크립트 완성(`backend/scripts/keyword-demand.mjs`, 2026-09-14), 단 네이버 검색광고 API 자격증명(`.env`) **발급만 남음**(searchad.naver.com 무료). (3) 사용자가 재개를 명시.
> 재개 후에도 목표는 "검색 유입 하루 1,000"이고 카덴스는 주 2~3편. 제휴링크(쿠팡파트너스 등)는 불허. 상세: `backend/blog/blogchart-plan.md` 2026-09-13 항목, `learnings.md`.
