...
**2026-09-13부터**: 같은 카테고리 최근 글과 소제목 3개 이상이 토씨까지 같으면 조립을 막고, 마무리
상투구("[날짜] 기준으로 정리했어요" · "다음 글에서는 ~ 다뤄볼게요" 류)가 최근 글 절반 이상과 겹치면
경고한다(`backend/blog/structure-log.json` 기록 기반 — 구글 서치콘솔에서 미색인 글 상당수가
"중복 페이지"로 잡힌 것의 대응). 반려·경고가 뜨면 소제목·마무리 문장을 실제로 바꿔라.
