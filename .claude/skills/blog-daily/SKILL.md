---
name: blog-daily
description: 오늘 쓸 블로그 글 10편(티스토리 5 + 네이버 5, 서로 다른 주제)을 스스로 고르고, /blog-pipeline 절차로 검증까지 돌린다. 발행은 하지 않는다 — 검증 통과분은 blog-queue.mjs(12:00·15:00·18:00 스케줄작업)가 플랫폼별로 순차 발행한다. 매일 09:00 Windows 스케줄작업 AZ2MZ_Blog_Daily(tools/blog-daily.vbs)가 이 스킬을 부른다.
---

# 블로그 일일 파이프라인 — 주제선정 → 검증까지 (발행은 대기열)

**이 스킬은 무인 실행을 전제로 한다.** 사람이 중간에 답할 수 없다. 판단이 갈리는 지점에서는
가장 보수적인 선택(그 항목을 보류하고 다음으로 넘어간다)을 해라 — 억지로 밀어붙이지 마라.

## 하루 목표 — 티스토리 5편 + 네이버 5편, 전부 다른 글 (2026-09-24 사용자 지시)

- **10편 모두 주제가 달라야 한다.** 같은 트렌드·같은 개념을 두 플랫폼에 나눠 쓰지 마라(각도만 바꾼
  같은 글도 안 됨 — 네이버 유사문서 판정·자기잠식). `blog-pick-trend.mjs` 는 양쪽 발행 기록과
  이미 써둔 초안(`backend/out/blog/<id>.json`)을 자동으로 빼준다. 키워드 백로그·자유 발굴 주제는
  `posted.json`·`posted-naver.json`·오늘 이미 고른 주제와 개념 단위로 직접 대조해라.
- **플랫폼 배정**: 각 글을 SEO 단계 뒤에 meta JSON(`backend/out/blog/<slug>.json`)에
  `"platform": "tistory"` 또는 `"platform": "naver"` 로 적는다. 네이버는 네이버 검색 유입이 강한
  생활밀착 주제(맛집·디저트·카페·핫플·신조어·패션·생활정보)를, 티스토리는 구글·다음 유입이 강한 정보성·기술
  주제(AI·개발·가이드)를 우선 배정하되 **각 5편**을 맞춘다. 소재가 한쪽에 몰리면 적합도가 낮은 쪽으로 넘겨서라도 5:5.
- **품질이 물량보다 먼저**: 검증을 통과 못 한 편은 채우려고 억지로 밀지 말고 보류한다(그날 5편 미만이 돼도 된다).
  조사 없이 쓰기·묵은/얇은 근거로 채우기는 여전히 금지.
- 편마다 아래 0~2단계를 반복한다. 서로 독립이니 **가능하면 서브에이전트로 병렬 처리**해라(편끼리 파일이 겹치지 않는다 — slug 가 다르다).
- 검증을 통과하면 `blog-verify.mjs` 가 meta 에 `verified`(본문 해시)를 남긴다. **대기열은 그게 있고 이후 본문이
  안 바뀐 글만 발행한다** — 검증 뒤에 본문을 고쳤으면 반드시 재조립·재검증해라.
...
- 플랫폼별(티스토리 N/5 · 네이버 N/5)로 편마다: 키워드·카테고리·최종 제목·`backend/out/blog/<slug>.html`
- 검증 결과(blocker/warn 개수, 있었던 재작성 라운드)
- `node backend/scripts/blog-queue.mjs --list` 결과(오늘 발행 대기 편수)
- **"발행: 12:00·15:00·18:00 스케줄작업 AZ2MZ_Blog_Queue 가 플랫폼별 하루 5편씩 순차 발행(로그 backend/out/blog/blog-queue.log). 티스토리 세션 만료 시 `blog-publish.mjs --login` 필요"** 안내를 마지막 줄에
